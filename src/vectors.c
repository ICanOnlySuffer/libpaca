...
static nil _vectors_at_quit () {
	for (u08 i = AT_QUIT.size - 1; i; i--) {
		((nil (*) ()) ((proc_t *) AT_QUIT.items [i]) -> proc) ();
	}
}
proc_t vectors_at_quit = PROC ("Vectors.at_quit", _vectors_at_quit);

static nil _vectors_quit () {
	vector_free (&ON_UPDATE);
	vector_free (&ON_EVENT);
	vector_free (&AT_QUIT);
}
static proc_t vectors_quit = PROC ("Vectors.quit", _vectors_quit);

static nil _vectors_init () {
	ON_UPDATE = vector_new ("ON_UPDATE", 4);
	ON_EVENT = vector_new ("ON_EVENT", 4);
	AT_QUIT = vector_new ("AT_QUIT", 4);
	at_quit_psh (&vectors_quit);
}
proc_t vectors_init = PROC ("Vectors.init", _vectors_init);
